 ...
 * Modified: Feb 10, 2019
 *
 * This is a camera class that was borrowed from the boilerplate code provided for CPSC 587
 */
...
Vec3f Camera::localFocalPoint() const { return m_pos + m_forward * m_focalDistance; }

float Camera::focusDistance() const { return m_focalDistance; }

void Camera::translate(Vec3f const &offset) {
    if (m_pos.m_y + offset.m_y <= 0.0) // lock the camera from going below the ground
        m_pos = math::Vec3f(m_pos.m_x + offset.m_x, m_pos.m_y, m_pos.m_z + offset.m_z);
    else
        m_pos += offset;
}
...
void Camera::moveForward(float distance) { translate(distance * localForward()); }

void Camera::moveBackward(float distance) { translate((-distance) * localForward()); }

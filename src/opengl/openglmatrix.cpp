...
Mat4f OrthographicProjection(float l, float r, float t, float b, float n, float f) {
    float a00 = 2.f / (r - l);
    float a03 = -(r + l) / (r - l);
    float a11 = 2.f / (t - b);
    float a13 = -(t + b) / (t - b);
...
Mat4f inverseFrustumProjection(float l, float r, float t, float b, float n, float f) {
    float a00 = (r - l) * (2.f * n);
    float a03 = (r + l) / (2.f * n);
    float a11 = (t - b) / (2.f * n);
    float a13 = (t + b) / (2.f * n);
...
Mat4f PerspectiveProjection(float fovDegrees, float aspectRatio, float zNear, float zFar) {
...
Mat4f inversePerspectiveProjection(float fovDegrees, float aspectRatio, float zNear, float zFar) {
    float top = std::tan(fovDegrees * (M_PI / 180.f) * (1.f / 2.f)) * zNear;
    float right = top * aspectRatio;
...
Mat4f inverseLookAtMatrix(const Vec3f &pos, const Vec3f &target, const Vec3f &up) {
    Vec3f f = pos - target; // inverted for R-handed CS
    f.normalize();
    Vec3f u = normalized(up);
    Vec3f r = normalized(u ^ f);
